...
- `ccg status ...`는 runtime mode/backend/model/port, route proof, tool-call integrity, 로컬 트래픽 요약을 보여줍니다.
- `ccg doctor ...`가 현재 상태의 기준 점검입니다. `[OK]`면 현재 체크는 정상입니다.
...
```mermaid
flowchart TD
  A["사용자가 ccg setup 실행"] --> B["bootstrap (scope config/state)"]
  ...
  H["사용자가 ccg model switch 실행"] --> I["모델 alias 정규화"]
  I --> J["config model 갱신"]
  J --> K["service 재설치/재시작"]
  K --> L["claude apply"]
  L --> M["doctor 검증"]
...
`ccg` 바이너리를 업데이트했다면, `service start` 전에 `ccg service install --vendor codex --profile default`를 1회 실행해 서비스 설정(macOS: launchd plist, Linux: systemd unit)을 재생성하세요.
...
예시: 과거의 잘못된/미지원 status 사용(`status --since 24h`, `ccg-status --since 24h`)으로
